  ...
  ERC677ReceiverInterface
{
  ...
  function fulfillRandomWords(Proof memory proof, RequestCommitment memory rc) external nonReentrant returns (uint96) {
    ...
    if (s_subscriptions[rc.subId].balance < payment) {
      console.log(
        "Insufficient balance for subId %s, balance %s, payment %s",
        rc.subId,
        s_subscriptions[rc.subId].balance,
        payment
      );
  ...
  function oracleWithdraw(address recipient, uint96 amount) external nonReentrant {
    if (s_withdrawableTokens[msg.sender] < amount) {
      console.log(
        "Insufficient withdrawable balance for oracle %s, balance %s, amount %s",
        msg.sender,
        s_withdrawableTokens[msg.sender],
        amount
      );
      revert InsufficientBalance();
    }
    ...
    if (!LINK.transfer(recipient, amount)) {
      console.log(
        "LINK transfer failed for oracle %s, recipient %s, amount %s",
        msg.sender,
        recipient,
        amount
      );
  ...
  function cancelSubscriptionHelper(uint64 subId, address to) private nonReentrant {
    ...
    if (!LINK.transfer(to, uint256(balance))) {
      console.log("LINK transfer failed for subId %s, to %s, amount %s", subId, to, balance);
      revert InsufficientBalance();
    }
